...
# v9.3.2 修复：首次下 small 模型需要联网，下载完后再加 offline
# 让 faster-whisper 自己处理 local_files_only，下载完成后会自动缓存
...
ASR_PRIMARY = 'whisper'  # v9.3.2: FunASR 在 Pi 5 8GB OOM Killer，换回 faster-whisper small（int8 量化）
...
def _init_asr():
    ...
    try:
        # v9.3.2: 首次跑需要联网下模型（~460MB），下完会自动缓存到 download_root
        # 后续跑可以改 local_files_only=True 提速
...
def main():
    ...
    log('=' * 50)
    log('  墩墩 v9.3.2 - faster-whisper small（int8）')
    log('  v9.3.2 修复: 关掉 HF_HUB_OFFLINE，让首次下模型能联网')
    log('  保留: 眼睛诊断 + 报日期 + 报名字 + 短期记忆')
    log('=' * 50)
